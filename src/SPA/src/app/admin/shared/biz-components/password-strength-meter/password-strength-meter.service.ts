...
export const DEFAULT_CONFIG: PSMOptions = {
  translations: zxcvbnOptions.translations
};
...
export class PasswordStrengthMeterService {
  ...
  scoreWithFeedback(password: string): {
    ...
    const result = zxcvbn(password);
    return { score: result.score, feedback: { suggestions: result.feedback.suggestions, warning: result.feedback.warning as string } };
  }
}
