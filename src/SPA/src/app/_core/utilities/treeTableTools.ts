import { TreeNodeInterface } from "@app/admin/shared/components/tree-table/tree-table.component";
    ...
    parents.forEach(parent => {
      ...
      children.forEach((current, index) => {
        ...
        if (current[fatherId] === parent.id) {
          ...
          // Remove the current child node from temp, and use temp as the new child node data. This is to reduce the number of child node traversals during recursion. The more levels of the parent-child relationship, the more advantageous it will be.
          temp.splice(index, 1);
...
// Add levels to the tree structure data and mark whether it is the root node. The root node isLeaf is true, and the level is represented by level.
...
// Get the flattened tree data, and the input parameter is treeData in map form.
